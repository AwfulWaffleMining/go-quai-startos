...
- **`mainHostId`, `rpcHostId`, their ports, and the health check ids are an API** consumed by quai-dashboard-startos. Renaming one breaks that package.
- **Keep the zone RPC opt-in.** go-quai's RPC has no authentication.
